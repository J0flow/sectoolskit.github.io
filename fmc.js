...
IoC / évidences :`;
    ...
    let resultHtml = introMessage.replace(/\n/g, '<br>');
...
--------------------------------------------------------------`;
    ...
    resultHtml += conclusionMessage.replace(/\n/g, '<br>');

    const ticketTitle = `[SOC UdeM][MOYEN]CISCO FMC - Security Intelligence Personnel vers Externe (10+) - ${hostname}`;
    resultHtml += `<br>Titre du ticket: ${ticketTitle} <br>CC : equipesoc@esitechnologies.com<br><br>--------------------------------------------------------------`;
